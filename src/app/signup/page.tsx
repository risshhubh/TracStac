    ...
    <div className="min-h-screen bg-brand-dark flex flex-col items-center justify-center p-6">
      ...
      <Card className="w-full max-w-md border border-white/5 shadow-2xl">
        <CardHeader className="text-center pb-2">
          <CardTitle className="text-2xl font-black italic uppercase tracking-tighter">Create Account</CardTitle>
          <p className="text-white/50 text-sm">Join the elite project management suite.</p>
        </CardHeader>
        <CardContent className="space-y-6 pt-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex bg-[#0a0a0a] rounded-none border border-white/10 p-1 shadow-[2px_2px_0px_0px_rgba(168,85,247,0.2)]">
              <button 
                type="button"
                onClick={() => setIsAdmin(false)}
                className={`flex-1 py-2 text-xs font-bold uppercase tracking-widest transition-all ${!isAdmin ? 'bg-brand-purple text-white' : 'text-white/50 hover:text-white'}`}
              >
                Employee
              </button>
              <button 
          ...
          <p className="text-center text-sm text-white/40">
            Existing User? <Link href="/login" className="text-brand-purple font-bold hover:underline cursor-pointer">Login Here</Link>
          </p>
        </CardContent>
      </Card>
    </div>
